...
def checkModelConvergence(models, _X):
    ...
    for i in range(nmodels - 1):
        for j in range(i + 1, nmodels):
            ...
            p += sum([1
                        for i in np.power(probX[probX_key[i]] - probX[probX_key[i]], 2).sum(axis=1).reshape(-1).tolist()
    ...
    return {
        ...
        'H0: distance_mean': 'Accept' if abs(t) < tt else 'Reject',
        'P(diff < delta)': p / (_X.shape[0] * (nmodels - 1) * (nmodels - 2)),
        'p(Accept H0: mean_1 = mean_2)': c / ((nmodels - 1) * (nmodels - 2))
    }